...
@pytest.mark.P0
def test_cex_fund_016_submit_withdraw(withdraw_api, account_api, cex_fund_test_data):
    """case_016: 提币提交 - 发起提币申请，校验余额扣减与风控"""
    log.step("case_016: 提币提交测试")
    data = cex_fund_test_data["withdraw"]["case_016"]
    ...
    log.success("✅ case_016 通过：提币提交正常，风控拦截有效")
...
def test_cex_fund_017_query_withdraw_progress(withdraw_api, cex_fund_test_data):
    ...
    data = cex_fund_test_data["withdraw"]["case_017"]

    # 1. 先提交一笔提币获取 withdraw_id
    result = withdraw_api.submit_withdraw(
        symbol=data["coin"],
        amount=data["amount"],
        address=data["address"],
    ...
    if result.get("error") or "id" not in result:
        ...
        log.success(f"✅ case_017 通过：提币历史查询正常（测试网限制，未实际提币）")
        return
    ...
    log.success(f"✅ case_017 通过：提币进度查询正常")


@pytest.mark.CEX_Fund
@pytest.mark.Withdraw
@pytest.mark.P0
def test_cex_fund_018_cancel_withdraw(withdraw_api, account_api, cex_fund_test_data):
    ...
    if result.get("error") or "id" not in result:
        ...
        log.success("✅ case_018 通过：提币接口可调用（测试网限制）")
        return
    ...
    log.success(f"✅ case_018 通过：提币撤销流程正常")
...
def test_cex_fund_019_get_withdraw_history(withdraw_api, cex_fund_test_data):
    ...
    log.success(f"✅ case_019 通过：提币历史查询正常，共 {len(all_history)} 条")
...
def test_cex_fund_020_low_gas_withdraw(withdraw_api, cex_fund_test_data):
    ...
    # 1. 尝试提币（测试网可能不支持自定义 Gas，但验证接口能正确处理）
    result = withdraw_api.submit_withdraw(
        symbol=data["coin"],
        amount=data["amount"],
        address=data["address"],
